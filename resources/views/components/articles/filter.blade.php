<div class="w-full flex rounded shadow">
    <a
        href="{{ route('articles', ['filter' => 'recent', 'tag' => $activeTag?->name()]) . '#articles' }}"
        aria-current="{{ $selectedFilter === 'recent' ? 'page' : 'false' }}"
        class="flex justify-center font-medium rounded-l px-5 py-2 border {{ $selectedFilter === 'recent' ? 'bg-gray-900 text-white  border-gray-900 hover:bg-gray-800' : 'bg-white text-gray-800 border-gray-200 hover:bg-gray-100' }}"
    >
        Reciente
    </a>
    ...
    <a
        ...
        class="flex justify-center font-medium px-5 py-2 border-t border-b {{ $selectedFilter === 'popular' ? 'bg-gray-900 text-white  border-gray-900 hover:bg-gray-800' : 'bg-white text-gray-800 border-gray-200 hover:bg-gray-100' }}"
    >
        Popular
    </a>
